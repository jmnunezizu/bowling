...
public class Frame {
    ...
    /**
     * Returns the number of the current throw. Every time pins are dropped, the
     * number of throws is incremented. The maximum number of throws will be
     * driven by {@link #getRequiredThrows()}.
     *
     * @return The number of the current throw.
     */
    ...
    public void dropPins(final int pinsDropped) {
        ...
        if (pinsDropped < 0 || pinsDropped > 10) {
            throw new InvalidNumberOfPinsException(String.valueOf(pinsDropped));
        }
        if (getPinsDroppedFirstThrow() + pinsDropped > getMaximumNumberOfDroppedPins()) {
            throw new InvalidNumberOfPinsException(String.valueOf(pinsDropped));
        }
    ...
    public String getDisplayValue() {
        ...
        if (isStrike()) {
            ...
            return getPinsDroppedFirstThrow() + " | " + getPinsDroppedSecondThrow();
        }
    }

    @Override
    public String toString() {
        return "Frame #" + (frames.getFrames().indexOf(this) + 1) + ", Throw #" + getCurrentThrow();
    }
